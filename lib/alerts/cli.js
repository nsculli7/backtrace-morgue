...
class AlertsCli {
  ...
  async routeMethod(args) {
    ...
    router.route(routes, "usage", args);
  }
  ...
  async generateAlertSpec(argv, isCreate) {
    ...
    const partial = {
      ...
      enabled: options.convertOne("enabled", argv.enabled),
      ...
      mute_until: convertOne("mute-until", argv['mute-until']),
  ...
  async updateAlert(argv) {
    ...
    await this.updateAlert(id, updated);
